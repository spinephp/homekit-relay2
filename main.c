...
homekit_characteristic_t switch_ons[] = {
    HOMEKIT_CHARACTERISTIC_( ON, false, .callback=HOMEKIT_CHARACTERISTIC_CALLBACK(switch_on_callback)),
    HOMEKIT_CHARACTERISTIC_(ON, false, .callback=HOMEKIT_CHARACTERISTIC_CALLBACK(switch_on_callback))
};
...
void gpio_init() {
    ...
    for(int i=0;i<relay_number; i++){
        relay_write(i,switch_ons[i].value.bool_value);
    }   
}

void set_relay_value(int index,bool value) {
    printf("Relay Value: %d\n", value);
    switch_ons[index].value.bool_value = value;
    relay_write(index,value);
    homekit_characteristic_notify(&switch_ons[index], switch_ons[index].value);
}

void toggle_relay_value(int index) {
    printf("Toggling relay\n");
    set_relay_value(index, !switch_ons[index].value.bool_value);
}
...
homekit_accessory_t *accessories[] = {
    HOMEKIT_ACCESSORY(.id=1, .category=homekit_accessory_category_switch, .services=(homekit_service_t*[]){
        ...
        HOMEKIT_SERVICE(SWITCH, .primary=true, .characteristics=(homekit_characteristic_t*[]){
            HOMEKIT_CHARACTERISTIC(NAME, "Sonoff Switch0"),
            &switch_ons[0],
            NULL
        } ),
        HOMEKIT_SERVICE(SWITCH, .primary=false, .characteristics=(homekit_characteristic_t*[]){
            HOMEKIT_CHARACTERISTIC(NAME, "Sonoff Switch1"),
            &switch_ons[1],
            NULL
        } ),
        NULL
...
{
    snprintf(pcInsert, iInsertLen, (switch_ons[0].value.bool_value) ? "on" : "off");
...
char *cgi_handler(int iIndex, int iNumParams, char *pcParam[], char *pcValue[])
{
    for (int i = 0; i < iNumParams; i++) {
        if (strcmp(pcValue[i], "on") == 0) {
            set_relay_value(i,true);
        } else if (strcmp(pcValue[i], "off") == 0) {
            set_relay_value(i,false);
        } else if (strcmp(pcValue[i], "toggle") == 0) {
            toggle_relay_value(i);
        }
    }
    return "/index.ssi";
}
...
void user_init(void) {
    ...
    wifi_config_init("sonoff-switch", NULL, on_wifi_ready,NULL);
    gpio_init();
    ...
    for(int i=0;i<relay_number;i++){
        set_relay_value(i,POWER_ON_STATE);
    }
}
